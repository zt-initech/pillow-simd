    ...
    for (y = 0; y < imDst->ysize; y++) {
        ...
        for (; x < xsize; x += 1) {
            if (src[x].a == 0) {
                ...
                UINT16 blend = dst[x].a * (255 - src[x].a);
                UINT16 outa255 = src[x].a * 255 + blend;
                // There we use 7 bits for precision.
                // We could use more, but we go beyond 32 bits.
                UINT16 coef1 = src[x].a * 255 * 255 * (1<<PRECISION_BITS) / outa255;
                UINT16 coef2 = 255 * (1<<PRECISION_BITS) - coef1;
